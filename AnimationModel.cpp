...
{
	//CreateVertexShader("skinningShaderVS.cso");
...
//void AnimationModel::LoadAnimation(const char * FileName, const char * Name)
//{
//	const aiScene* as = aiImportFile(FileName, aiProcessPreset_TargetRealtime_MaxQuality | aiProcess_ConvertToLeftHanded);
//	assert(as);
//	m_Animation[Name] = as;
//}
...
void AnimationModel::Update(const char* AnimationName, const char* AnimationName2, float BlendRate, int Frame)
{
	...
	for (unsigned int m = 0; m < m_AiScene->mNumMeshes; m++) {
		...
		for (unsigned int v = 0; v < mesh->mNumVertices; v++) {
			...
			//deformVertex->Position = outMatrix * deformVertex->Position;
